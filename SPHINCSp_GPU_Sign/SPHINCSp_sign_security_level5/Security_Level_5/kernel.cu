...
//CORE
__global__ void GPU_fors_sign_throughput_security_level_5_oneblock(uint8_t* sig, uint8_t* roots, uint32_t* indices, uint8_t* sk_seed, uint8_t* pub_seed, uint32_t fors_addr[8], uint8_t* state_seed, uint32_t* lengths) {
    __shared__ uint8_t shared_stack[HASH_DIGEST * (1 << (FORS_HEIGHT - 1))];
    uint8_t iternal_pub_seed[PK_BYTE] = { 0, };
    uint8_t iternal_sk_seed[SK_BYTE] = { 0, };
    uint8_t iternal_state_seed[HASH_OUTBYTE + 8] = { 0, };
    uint8_t root[HASH_DIGEST] = { 0, };
    uint8_t stack[HASH_DIGEST * 2];
    ...
    for (int i = 0; i < FORS_TREE; i++) {
        ...
        //LEAF GEN
        GPU_fors_gen_leaf(stack, iternal_sk_seed, iternal_pub_seed, 2 * threadIdx.x + idx_offset, fors_tree_addr, iternal_state_seed);
        GPU_fors_gen_leaf(stack + HASH_DIGEST, iternal_sk_seed, iternal_pub_seed, 2 * threadIdx.x + 1 + idx_offset, fors_tree_addr, iternal_state_seed);
        if ((leaf_idx ^ 0x1) == 2 * threadIdx.x) {
            memcpy(sig + HASH_DIGEST + sig_index, stack, HASH_DIGEST);
        }
        else if ((leaf_idx ^ 0x1) == (2 * threadIdx.x + 1)) {
            memcpy(sig + HASH_DIGEST + sig_index, stack + HASH_DIGEST, HASH_DIGEST);
        }

        //FORS Depth 1 [512->256]
        GPU_set_tree_height(fors_tree_addr, 1);
        GPU_set_tree_index(fors_tree_addr, threadIdx.x + (idx_offset >> 1));
        GPU_tree_thash_2depth(shared_stack + (HASH_DIGEST * threadIdx.x), stack, stack + HASH_DIGEST, iternal_pub_seed, fors_tree_addr, iternal_state_seed);
        if (((leaf_idx >> 1) ^ 0x1) == threadIdx.x)
            memcpy(sig + 2 * HASH_DIGEST + sig_index, shared_stack + (HASH_DIGEST * threadIdx.x), HASH_DIGEST);
        __syncthreads();
        ...
        if (threadIdx.x < 128) {
            ...
            GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x), shared_stack + (2 * HASH_DIGEST * threadIdx.x), shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST,
                iternal_pub_seed, fors_tree_addr, iternal_state_seed);
            if (((leaf_idx >> 2) ^ 0x1) == threadIdx.x)
                memcpy(sig + 3 * HASH_DIGEST + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x), HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 64) {
            ...
            if (((leaf_idx >> 3) ^ 0x1) == threadIdx.x)
                memcpy(sig + 4 * HASH_DIGEST + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST, HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 32) {
            ...
            if (((leaf_idx >> 4) ^ 0x1) == threadIdx.x)
                memcpy(sig + 5 * HASH_DIGEST + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x), HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 16) {
            ...
            if (((leaf_idx >> 5) ^ 0x1) == threadIdx.x)
                memcpy(sig + 6 * HASH_DIGEST + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST, HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 8) {
            ...
            if (((leaf_idx >> 6) ^ 0x1) == threadIdx.x)
                memcpy(sig + 7 * HASH_DIGEST + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x), HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 4) {
            ...
            if (((leaf_idx >> 7) ^ 0x1) == threadIdx.x)
                memcpy(sig + 8 * HASH_DIGEST + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST, HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 2) {
            ...
            GPU_set_tree_index(fors_tree_addr, threadIdx.x + (idx_offset >> 8));
            GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x), shared_stack + (4 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST,
                shared_stack + (4 * HASH_DIGEST * threadIdx.x) + 3 * HASH_DIGEST, iternal_pub_seed, fors_tree_addr, iternal_state_seed);
            if (((leaf_idx >> 8) ^ 0x1) == threadIdx.x)
                memcpy(sig + 9 * HASH_DIGEST + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x), HASH_DIGEST);
        }
        __syncthreads();

        if (threadIdx.x == 0) {
            GPU_set_tree_height(fors_tree_addr, 9);
            GPU_set_tree_index(fors_tree_addr, threadIdx.x + (idx_offset >> 9));
            GPU_tree_thash_2depth(roots + (FORS_TREE * HASH_DIGEST * blockIdx.x) + (HASH_DIGEST * i), shared_stack, shared_stack + 2 * HASH_DIGEST, iternal_pub_seed,
                fors_tree_addr, iternal_state_seed);
        }
        sig_index = sig_index + (10 * HASH_DIGEST);
    }

    if (threadIdx.x == 0) {
        GPU_fors_final_thash(root, roots + (HASH_DIGEST * FORS_TREE * blockIdx.x), iternal_pub_seed, fors_pk_addr, state_seed);
        GPU_chain_lengths(lengths + blockIdx.x * (WOTS_LEN * (SUBTREE_LAYER + 1)), root);
    }
}

// SUB_LAYER -> 17, (1<< TREE_HEIGHT) = 16
__global__ void GPU_sphincs_tree_hash_security_level_5_oneblock(uint32_t* lengths, uint8_t* sig, uint8_t* sk_seed, uint8_t* pub_seed, uint8_t* state_seed, uint32_t* leaf_idx, uint64_t* tree) {
    ...
    uint32_t sphincs_leaf[SUBTREE_LAYER] = { 0, };
    uint64_t sphincs_tree[SUBTREE_LAYER] = { 0, };
    ...
    GPU_set_type(tree_addr, 2);
    GPU_set_layer_addr(tree_addr, iternal_layeridx);
    GPU_set_tree_addr(tree_addr, sphincs_tree[iternal_layeridx]);

    GPU_wots_gen_leaf(shared_stack + HASH_DIGEST * threadIdx.x, iternal_sk_seed, iternal_pub_seed, iternal_state_seed, (sphincs_idx_offset + (threadIdx.x % 16)), tree_addr);
    if (((sphincs_leaf[iternal_layeridx]) ^ 0x1) == (threadIdx.x % 16)) {
        memcpy(sig + sig_store_index, shared_stack + HASH_DIGEST * threadIdx.x, HASH_DIGEST);
    }
    __syncthreads();

    if (threadIdx.x < (blockDim.x >> 1)) {
        iternal_layeridx = (threadIdx.x / 8);
        sig_store_index = ((iternal_layeridx + 1)) * (WOTS_BYTES + (HASH_DIGEST * TREE_HEIGHT)) - (HASH_DIGEST * TREE_HEIGHT) + (blockIdx.x * SUBTREE_LAYER * (WOTS_BYTES + TREE_HEIGHT * HASH_DIGEST));
        GPU_set_layer_addr(tree_addr, iternal_layeridx);
        GPU_set_tree_addr(tree_addr, sphincs_tree[iternal_layeridx]);

        GPU_set_tree_height(tree_addr, 1);
        GPU_set_tree_index(tree_addr, (threadIdx.x % 8) + (sphincs_idx_offset >> 1));
        GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x), shared_stack + (2 * HASH_DIGEST * threadIdx.x),
            shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST, iternal_pub_seed, tree_addr, iternal_state_seed);

        if (((sphincs_leaf[iternal_layeridx] >> 1) ^ 0x1) == (threadIdx.x % 8))
    ...
    if (threadIdx.x < (blockDim.x >> 2)) {
        ...
        GPU_set_tree_index(tree_addr, (threadIdx.x % 4) + (sphincs_idx_offset >> 2));
        GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST, shared_stack + (4 * HASH_DIGEST * threadIdx.x),
            shared_stack + (4 * HASH_DIGEST * threadIdx.x) + 2 * HASH_DIGEST, iternal_pub_seed, tree_addr, iternal_state_seed);
        if (((sphincs_leaf[iternal_layeridx] >> 2) ^ 0x1) == threadIdx.x % 4)
    ...
    if (threadIdx.x < (blockDim.x >> 3)) {
        ...
        GPU_set_tree_height(tree_addr, 3);
        GPU_set_tree_index(tree_addr, (threadIdx.x % 2) + (sphincs_idx_offset >> 3));

        GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x), shared_stack + (4 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST,
            shared_stack + (4 * HASH_DIGEST * threadIdx.x) + (3 * HASH_DIGEST), iternal_pub_seed, tree_addr, iternal_state_seed);

        if (((sphincs_leaf[iternal_layeridx] >> 3) ^ 0x1) == threadIdx.x % 2)
            memcpy(sig + sig_store_index + 3 * HASH_DIGEST, shared_stack + (2 * HASH_DIGEST * threadIdx.x), HASH_DIGEST);
    }

    if (threadIdx.x < SUBTREE_LAYER) {
        ...
        GPU_set_tree_height(tree_addr, 4);
        GPU_set_tree_index(tree_addr, (sphincs_idx_offset >> 4));
        GPU_tree_thash_2depth(sphincs_root, shared_stack + (4 * HASH_DIGEST * threadIdx.x), shared_stack + (4 * HASH_DIGEST * threadIdx.x) + (2 * HASH_DIGEST),
            iternal_pub_seed, tree_addr, iternal_state_seed);
        GPU_chain_lengths(test_lengths, sphincs_root);
        memcpy(lengths + WOTS_LEN + (WOTS_LEN * iternal_layeridx) + (WOTS_LEN * blockIdx.x * (SUBTREE_LAYER + 1)), test_lengths, WOTS_LEN * 4);
    }
}
// 10(SUB_LAYER-2) * 67(WOTS_LEN)
__global__ void GPU_sphincs_wots_sign_security_level_5_oneblock(uint8_t* sig, uint32_t* lengths, uint8_t* sk_seed, uint8_t* pub_seed, uint8_t* state_seed, uint32_t* leaf_idx, uint64_t* tree) {

    uint8_t hash_temp[HASH_DIGEST] = { 0, };
    uint8_t iternal_pub_seed[PK_BYTE] = { 0, };
    uint8_t iternal_sk_seed[SK_BYTE] = { 0, };
    uint8_t iternal_state_seed[40] = { 0, };
    ...
    //LAYER 10 ~ 16
    if (threadIdx.x < (7 * WOTS_LEN)) {
        iternal_layer = (threadIdx.x / WOTS_LEN) + 10;
        sig_store_index = ((iternal_layer) * (WOTS_BYTES + (TREE_HEIGHT * HASH_DIGEST))) + blockIdx.x * (SIG_BYTE - FORS_BYTES - HASH_DIGEST);
        iternal_tree = tree[0];
        iternal_leaf_idx = leaf_idx[0];
...
int crypto_sign_signature_security_level_5(uint8_t* sig, size_t* siglen, uint8_t* m, size_t mlen, uint8_t* sk, uint32_t msgNum) {
    uint8_t* sk_seed = sk;
    ...
    cudaMalloc((void**)&gpu_fors_sig, sizeof(uint8_t) * FORS_BYTES);
    cudaMalloc((void**)&gpu_root, FORS_TREE * sizeof(uint8_t) * HASH_DIGEST);
    ...
    cudaMalloc((void**)&gpu_indices, sizeof(uint32_t) * FORS_TREE);
    cudaMalloc((void**)&gpu_lengths, sizeof(uint32_t) * WOTS_LEN * (SUBTREE_LAYER + 1));

    cudaMemcpy(gpu_indices, indices, sizeof(uint32_t) * FORS_TREE, cudaMemcpyHostToDevice);
    ...
    uint8_t* gpu_throughput_roots = NULL;
    ...
    uint8_t* cpu_wots_sign = (uint8_t*)malloc(msgNum * SUBTREE_LAYER * (WOTS_BYTES + HASH_DIGEST * TREE_HEIGHT));

    //cudaMalloc
    cudaMalloc((void**)&gpu_fors_throughput_test, msgNum * FORS_BYTES);
    cudaMalloc((void**)&gpu_throughput_roots, msgNum * HASH_DIGEST * FORS_TREE);
    ...
    cudaError_t err;
    GPU_fors_sign_throughput_security_level_5_oneblock << <msgNum, 256 >> > (gpu_fors_throughput_test, gpu_throughput_roots, gpu_fors_throughput_indices, gpu_sk_seed, gpu_pub_seed, gpu_wots_addr, gpu_state_seed, gpu_fors_throughput_lengths);
    cudaMemcpy(cpu_fors_sign, gpu_fors_throughput_test, msgNum * FORS_BYTES, cudaMemcpyDeviceToHost);
    GPU_sphincs_tree_hash_security_level_5_oneblock << <msgNum, SUBTREE_LAYER* (1 << TREE_HEIGHT) >> > (gpu_fors_throughput_lengths, gpu_wots_sig, gpu_sk_seed, gpu_pub_seed, gpu_state_seed, gpu_idx_leaf, gpu_tree);
    ...
    for (int i = 0; i < 100; i++) {
        GPU_sphincs_wots_sign_security_level_5_oneblock << <msgNum, (10 * WOTS_LEN) >> > (gpu_wots_sig, gpu_fors_throughput_lengths, gpu_sk_seed, gpu_pub_seed, gpu_state_seed, gpu_idx_leaf, gpu_tree);
...
int main() {
    ...
    for (int i = 0; i < 10; i++) {
        crypto_sign_signature_security_level_5(sig, &siglen, m, SK_BYTE, sk, msgNum[i]);
    }
}
