...
//for BankConflict
__global__ void GPU_fors_sign_throughput_security_level_1_oneblock_bc(uint8_t* sig, uint32_t* indices, uint8_t* sk_seed, uint8_t* pub_seed, uint32_t fors_addr[8], uint8_t* state_seed, uint32_t* lengths) {
    ...
    for (int i = 0; i < FORS_TREE; i++) {
        ...
        GPU_fors_gen_leaf(shared_stack + (HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x/8), iternal_sk_seed, iternal_pub_seed, threadIdx.x + idx_offset, fors_tree_addr, iternal_state_seed);

        if ((leaf_idx ^ 0x1) == threadIdx.x)
            memcpy(sig + HASH_DIGEST + sig_index, shared_stack + (HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 8), HASH_DIGEST);
        __syncthreads();
        ...
        if (threadIdx.x < 32) {
            ...
            GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 4), shared_stack + (2 * HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 4),
                shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST + 4 * (threadIdx.x / 4), iternal_pub_seed, fors_tree_addr, iternal_state_seed);
            if (((leaf_idx >> 1) ^ 0x1) == threadIdx.x) {
                memcpy(sig + (2 * HASH_DIGEST) + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 4), HASH_DIGEST);
            }
        }
        __syncthreads();
        ...
        if (threadIdx.x < 16) {
            ...
            GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST + 4 * (threadIdx.x / 4), shared_stack + (4 * HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 2),
                shared_stack + (4 * HASH_DIGEST * threadIdx.x) + 2 * HASH_DIGEST + 4 * (threadIdx.x / 2), iternal_pub_seed, fors_tree_addr, iternal_state_seed);

            if (((leaf_idx >> 2) ^ 0x1) == threadIdx.x)
                memcpy(sig + (3 * HASH_DIGEST) + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST + 4 * (threadIdx.x / 4), HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 8) {
            ...
            GPU_tree_thash_2depth(shared_stack + (2 * HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 4), shared_stack + (4 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST + 4 * (threadIdx.x / 2),
                shared_stack + (4 * HASH_DIGEST * threadIdx.x) + 3 * HASH_DIGEST + 4 * (threadIdx.x / 2), iternal_pub_seed, fors_tree_addr, iternal_state_seed);
            if (((leaf_idx >> 3) ^ 0x1) == threadIdx.x)
                memcpy(sig + (4 * HASH_DIGEST) + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 4), HASH_DIGEST);
        ...
        if (threadIdx.x < 4) {
            ...
            if (((leaf_idx >> 4) ^ 0x1) == threadIdx.x)
                memcpy(sig + (5 * HASH_DIGEST) + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + HASH_DIGEST + 4 * (threadIdx.x / 4), HASH_DIGEST);
        }
        __syncthreads();
        ...
        if (threadIdx.x < 2) {
            ...
            if (((leaf_idx >> 5) ^ 0x1) == threadIdx.x)
                memcpy(sig + (6 * HASH_DIGEST) + sig_index, shared_stack + (2 * HASH_DIGEST * threadIdx.x) + 4 * (threadIdx.x / 4), HASH_DIGEST);
        }
        ...
        if (threadIdx.x == 0) {
            ...
            GPU_tree_thash_2depth(root + (HASH_DIGEST * i), shared_stack + 4 * (threadIdx.x / 8), shared_stack + 2 * HASH_DIGEST + 4 * (threadIdx.x / 4),
...
int main() {
    ...
    crypto_sign_signature_security_level_1(sig, &siglen, m, SK_BYTE, sk, 512);
}
