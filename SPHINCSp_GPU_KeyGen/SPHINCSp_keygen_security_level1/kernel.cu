...
__global__ void fors_sign_latency(uint8_t* sig, uint8_t* roots, uint32_t* indices, uint8_t* sk_seed, uint8_t* pub_seed, uint32_t fors_addr[8], uint8_t* state_seed, uint32_t* lengths) {
    ...
    if (threadIdx.x == 0) {
        ...
        set_tree_index(fors_tree_addr, idx_offset >> 6);
        tree_thash_2depth(roots + (SPX_N * blockIdx.x), shared_stack, shared_stack + 2 * SPX_N,
...
__global__ void crypto_sign_keypair(uint8_t* iternal_seed, uint8_t* pk, uint8_t* sk, uint8_t* state) {
    ...
    if (threadIdx.x == 0) {
        ...
        randombytes_init(info, iternal_seed);
        randombytes(info, iternal_seed, CRYPTO_SEEDBYTES);
...
void crypto_sign_keypair_test(int blocksize) {
    ...
    uint8_t seed_input[CRYPTO_SEEDBYTES] = { 0x06, 0x15 , 0x50 , 0x23 , 0x4D , 0x15 , 0x8C , 0x5E , 0xC9 , 0x55 , 0x95 , 0xFE , 0x04 , 0xEF , 0x7A , 0x25 , 0x76 , 0x7F , 0x2E , 0x24 , 0xCC , 0x2B , 0xC4 , 0x79 , 0xD0 , 0x9D , 0x86 , 0xDC , 0x9A , 0xBC , 0xFD , 0xE7 , 0x05 , 0x6A , 0x8C , 0x26 , 0x6F , 0x9E , 0xF9 , 0x7E , 0xD0 , 0x85 , 0x41 , 0xDB , 0xD2 , 0xE1 , 0xFF , 0xA1};//{ 0, };

    uint8_t* cpu_seed = (uint8_t*)malloc(CRYPTO_SEEDBYTES * blocksize);

    for (int i = 0; i < blocksize; i++) {
        memcpy(cpu_seed+i* CRYPTO_SEEDBYTES,seed_input, CRYPTO_SEEDBYTES);
    }

    uint8_t* cpu_pk = (uint8_t*)malloc(SPX_PK_BYTES*blocksize);
    uint8_t* cpu_sk = (uint8_t*)malloc(SPX_SK_BYTES*blocksize);
    ...
    cudaMemcpy(gpu_seed, seed_input, CRYPTO_SEEDBYTES, cudaMemcpyHostToDevice);
    ...
    cudaMemcpy(cpu_sk, gpu_sk, SPX_SK_BYTES * blocksize, cudaMemcpyDeviceToHost);
    cudaMemcpy(cpu_pk, gpu_pk, SPX_PK_BYTES * blocksize, cudaMemcpyDeviceToHost);

    elapsed_time_ms /= 1;
    elapsed_time_ms = 1000 / elapsed_time_ms;
    elapsed_time_ms = elapsed_time_ms * blocksize;
    //printf("sign_keypair = %4.2f ms\n", elapsed_time_ms);
    printf("%d's SPHINCS+ = %4.2f kops\n", blocksize, elapsed_time_ms / 1000);


    for (int i = 0; i < SPX_PK_BYTES * blocksize; i++) {
        printf("%02X", cpu_pk[i]);

        if ((i % SPX_PK_BYTES) == (SPX_PK_BYTES - 1)) {
            printf("\n");
        }
    }

    for (int i = 0; i < SPX_SK_BYTES * blocksize; i++) {
        printf("%02X", cpu_sk[i]);

        if ((i % SPX_SK_BYTES) == (SPX_SK_BYTES - 1)) {
            printf("\n");
        }
    }
}
